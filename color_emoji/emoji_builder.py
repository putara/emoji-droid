...
class CBDT:
  ...
  def write_glyphMetrics(self, width, height, big_metrics):
    ...
    try:
      ...
      raise ValueError("%s, h: %d w: %d x: %d y: %d vx: %d vy: %d a: %d" % (
          e, height, width, x_bearing, y_bearing, vert_x_bearing, vert_y_bearing, advance))
...
""", file=sys.stderr)
  ...
  for img_prefix in img_prefixes:
    ...
    img_files = {}
    glb = os.path.join(img_prefix, '*.png')
